...
@section('content')
    <div class="container">
        <h1 class="mb-4 text-center" style="font-weight: 600; color: #1e3a8a;">Créer un émargement</h1>
        ...
        <div class="card shadow-sm">
            <div class="card-body">
                <form action="{{ route(auth()->user()->role . '.emargements.store') }}" method="POST">
                    @csrf
                    <div class="row g-3">
                                ...
                                <div class="input-group">
                                    ...
                                    <select name="cours_professeur" id="cours_professeur" class="form-control" required>
                                        ...
                                        @foreach ($coursProfesseurOptions as $option)
                                            <option value="{{ $option['value'] }}" {{ old('cours_professeur') == $option['value'] ? 'selected' : '' }}>
                                                {{ $option['label'] }}
                                            </option>
                                        @endforeach
                                    </select>
